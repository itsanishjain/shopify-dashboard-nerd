...
import { getTopCustomersBySpending, getTopCustomersByOrders, CustomerItem } from "@/data/customerData";
...
const CustomerTableRow = ({ customer, showOrders = false }: { customer: CustomerItem, showOrders?: boolean }) => {
  return (
    <TableRow>
      <TableCell>
      ...
      <TableCell>
        <Badge variant={customer.tier === "VIP" ? "default" : customer.tier === "Regular" ? "secondary" : "outline"}>
          {customer.tier}
        </Badge>
      </TableCell>
...
const TopCustomersTable: React.FC = () => {
  ...
  return (
    <Card className="col-span-12 matrix-flow shadow-glow-sm">
      <CardHeader>
        <CardTitle className="text-xl font-semibold terminal-text">Top Customers</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {topOrderCustomers.map((customer) => (
                    <CustomerTableRow key={customer.id} customer={customer} showOrders={true} />
                  ))}
                </TableBody>
              </Table>
            </div>
