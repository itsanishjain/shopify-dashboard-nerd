...
export const getSalesByLocation = () => {
  const locations = [
    ...
    "Seattle", "Boston", "Denver", "Austin", "San Francisco"
  ];
...
export const getPeakSalesTimes = () => {
  ...
  days.forEach(day => {
    hours.forEach(hour => {
      ...
      const boost = (hour >= 12 && hour <= 13) || (hour >= 17 && hour <= 19) ? 1.5 : 1;
      // Weekend boost
      const weekendBoost = (day === "Saturday" || day === "Sunday") && hour >= 11 && hour <= 16 ? 1.3 : 1;
...
export const getDiscountCodePerformance = () => {
  const generateDiscountCode = () => {
    const prefixes = ["SAVE", "SUMMER", "SPECIAL", "NEW", "WELCOME", "FLASH", "HOLIDAY"];
  ...
  return Array.from({ length: 10 }, () => {
    ...
    const revenueGenerated = usageCount * faker.number.int({ min: 50, max: 150 });
...
export const getOrderStatusData = () => {
  ...
  const avgOrderValue = faker.number.float({ min: 70, max: 90, precision: 0.01 });
